...
from agent_orchestrator.server import create_app


def _poll_until(client: TestClient, quick_action_id: str, target_statuses: set[str], timeout: float = 5.0) -> dict:
    """Poll GET /api/quick-actions/{id} until status is in target_statuses."""
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        resp = client.get(f"/api/quick-actions/{quick_action_id}")
...
def test_post_returns_queued(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        resp = client.post("/api/quick-actions", json={"prompt": "explain something"})
        assert resp.status_code == 200
        qa = resp.json()["quick_action"]
...
def test_shortcut_via_api(tmp_path: Path) -> None:
    app = create_app(project_dir=tmp_path)
    with TestClient(app) as client:
        resp = client.post("/api/quick-actions", json={"prompt": "git status"})
        qa_id = resp.json()["quick_action"]["id"]
        result = _poll_until(client, qa_id, {"completed", "failed"})
...
def test_promotion_still_works_immediately(tmp_path: Path) -> None:
    ...
    app = create_app(project_dir=tmp_path)
    with TestClient(app) as client:
        resp = client.post("/api/quick-actions", json={"prompt": "do something"})
        qa_id = resp.json()["quick_action"]["id"]
        promote = client.post(f"/api/quick-actions/{qa_id}/promote", json={})
